...
async function upsert(col: string, id: string, data: Record<string, any>) {
  await db.collection(col).doc(id).set({ id, ...data }, { merge: true });
}
...
async function seed() {
  ...
  await upsert("Events", event1Id, {
    ...
    notes: "",
    duration: 30,
    foodArrived: admin.firestore.Timestamp.fromDate(new Date(now.getTime() - 30 * 60 * 1000)),
    foodAvailable: admin.firestore.Timestamp.fromDate(new Date(now.getTime() - 10 * 60 * 1000)),
  ...
  await upsert("Events", event2Id, {
    ...
    duration: 30,
    foodArrived: admin.firestore.Timestamp.fromDate(new Date(now.getTime() - 90 * 60 * 1000)),
    foodAvailable: admin.firestore.Timestamp.fromDate(new Date(now.getTime() - 60 * 60 * 1000)),
  ...
  await db.collection("Reviews").doc(event1Id)
    .collection("Reviews")
    .doc("r1")
    .set({
      id: "r1",
