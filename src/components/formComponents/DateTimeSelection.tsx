...
export const DateTimeSelection: React.FC<DateTimeSelectionProps> = ({
  ...
  const [duration, setDuration] = useState<Date>(
    new Date(0, 0, 0, 0, defaultDuration, 0),
  );

  const maxTime = 4; // Maximum time in hours for food to be consumed after arrival
  const [availabilityLimit, setLimit] = useState<Date>(
    new Date(
      foodArrived.getTime() +
        maxTime * 60 * 60 * 1000 -
  ...
  const dynamicMaxTime = new Date(
    0,
    0,
    0,
    maxTime -
      (foodAvailable.getTime() - foodArrived.getTime()) / (60 * 60 * 1000),
    0,
    0,
  );
          ...
          <DateTimePicker
            ...
            slotProps={{
              textField: {
                fullWidth: true,
                helperText:
                  "For safety, food can't be given after 4 hours of arrival",
              },
            }}
            sx={{ ...props }}
          />
