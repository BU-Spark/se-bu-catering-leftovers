...
export const FoodSelection: React.FC<FoodSelectionProps> = ({
  ...
  return (
    <Grid
      ...
      borderRadius="12px"
    >
      <Typography paddingLeft={2} fontSize="1.2rem" color="secondary">
        Select Food Items
      </Typography>
        ...
        >
              ...
              >
                {unitOptions.map((unit) => (
                  <MenuItem key={unit} value={unit}>
                    <Typography paddingTop={'1.5px'} textAlign="left">
                      {unit}
                    </Typography>
                  </MenuItem>
                ))}
          ...
          <Grid item marginLeft={1} xs>
            <StyledTextField
              ...
              value={item.item}
              onChange={(e) =>
                handleFoodItemChange(item.id, 'item', e.target.value)
              }
            />
          </Grid>
        </Grid>
