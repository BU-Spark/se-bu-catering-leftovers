...
const defaultImageUrl =
  'https://firebasestorage.googleapis.com/v0/b/bu-catering-leftovers.appspot.com/o/BUCL%20Default.jpeg?alt=media&token=e3b16eef-c37e-4407-85eb-f48cd1b501c2';
...
const EventForm: React.FC<EventFormProps> = ({ event, onPublish }) => {
  ...
  const [location, setLocation] = useState<Location>(
    event.Location ? event.Location : defaultAddress,
  );
  ...
  const isValid = () => {
    ...
    if (missingFields.length > 0) {
      alert(
        `Please fill out all required fields before publishing: ${missingFields.join(', ')}`,
      );
      return false;
    } else {
      return true;
  ...
  const updateEvent = (status: string) => {
    const validFood = foodItems.filter(
      ({ quantity, unit, item }) => quantity && unit && item,
    ); // Filter empty food items
  ...
  return (
    <div style={{ background: '#FFF6EE' }}>
      <Container
        maxWidth="sm"
        style={{ padding: '1em', paddingTop: '7em', background: '#FFF6EE' }}
      >
